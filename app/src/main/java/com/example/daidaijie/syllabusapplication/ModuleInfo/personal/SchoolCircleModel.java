...
//                                if (thumbUpsBean.getUid() == mIUserModel.getUserBaseBeanNormal().getId()) {
    ...
    @Override
    public Observable<List<CommnetBean>> getCommnetFromNet(int page_index) {
        int uid = mIUserModel.getUserInfoNormal().getUser_id();
        String token = mIUserModel.getUserInfoNormal().getToken();
        return mPersonalApi.getPersonalComment(3,"100002", page_index)
                .subscribeOn(Schedulers.io())
                .flatMap(new Func1<PersonalHttpBean, Observable<List<CommnetBean>>>() {
                    @Override
                    public Observable<List<CommnetBean>> call(PersonalHttpBean infoHttpBean) {
                        List<CommnetBean> post_list = infoHttpBean.getData();
                        mCommentListBeen = post_list;
                        return Observable.just(post_list);
                    }
                })
                .observeOn(AndroidSchedulers.mainThread());
    ...
    @Override
    public Observable<List<CommnetBean>> loadCommnetFromNet(int page_index) {
        return getCommnetFromNet(page_index);
    }
...
//                mIUserModel.getUserInfoNormal().getToken())
    ...
    @Override
    public Observable<List<CommnetBean>> deleteComment(final int position) {
        final CommnetBean postListBean = mCommentListBeen.get(position);
        UserInfo userInfo = mIUserModel.getUserInfoNormal();
//        userInfo.getUser_id(), userInfo.getToken())
        return mPersonalApi.deleteComment(postListBean.getId(),
                3, "100002")
                .subscribeOn(Schedulers.io())
                .flatMap(new Func1<InfoDeleteReturn, Observable<List<CommnetBean>>>() {
                    @Override
                    public Observable<List<CommnetBean>> call(InfoDeleteReturn voidHttpResult) {
                        if (voidHttpResult != null) {
